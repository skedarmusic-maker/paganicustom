...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-0 w-full z-50 bg-black/90 backdrop-blur-md border-b-2 border-primary/20 transition-all duration-300">
      ...
      {/* Mobile Menu Overlay */}
      <div className={`fixed inset-0 bg-black z-40 transition-all duration-500 md:hidden ${isOpen ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'}`}>
        <div className="flex flex-col items-center justify-center h-full space-y-8 p-4">
          <div className="absolute top-0 left-0 w-full h-full overflow-hidden select-none pointer-events-none opacity-[0.05] z-0 flex items-center justify-center">
             <div className="text-[30vw] font-black outline-text text-transparent transform rotate-90" style={{ WebkitTextStroke: '2px white' }}>
               PAGANI
             </div>
          </div>
          
          <nav className="relative z-10 flex flex-col items-center space-y-6 w-full px-6">
            {navLinks.map((link) => (
              <Link 
                key={link.href}
                href={link.href}
            ...
            <a
              ...
              rel="noopener noreferrer"
              onClick={() => setIsOpen(false)}
              className="w-full text-center bg-primary text-black font-heading font-bold text-lg tracking-widest uppercase py-4 border-2 border-primary shadow-[0_0_20px_rgba(245,231,32,0.4)] mt-8"
            >
              Agendar Agora
            </a>
